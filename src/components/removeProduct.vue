    ...
    <table class="table">
      ...
      <tbody>
        ...
        <td><img v-bind:src="product.image" /></td> 
          ...
          <td>{{ product.rate }}</td>
          <td>{{ product.count }}</td>
          <td><button v-on:click="remove(product.id)">Remove Product</button></td>
          <td><button v-on:click="update(product.id)">Update Product</button></td>
        </tr>
      </tbody>
    </table>
  </div>
...
export default {
    ...
    methods:{
        async remove(idd){
            let result=await axios.post(`http://localhost:5500/removeproduct/${idd}`)
            alert(result.data)
